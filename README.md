...
- **RAG**：md/txt/pdf → 结构感知切块（按 markdown 标题）→ litellm embedding → sqlite-vec；Analyst/Modeler/Writer prompt 自动注入（受 `MATH_AGENT_RAG_ENABLED` 控制）。语料分 `corpus/models/`（内置经典模型片段）与 `corpus/papers/`（自备历年论文）两类，writer 优先检索论文
