...
Q3 的局部事件响应先将一个受影响任务移出原路线，枚举其他车辆的容量可行插入位置，以道路距离与新增晚到联合增量最小者重插；再分别构造订单取消、新增订单、地址替换、时间窗收紧和车辆故障情景。每类情景独立从静态主方案开始，输出类型成功率和总体回退率；该实验验证五类单事件压力，不冒充连续多事件滚动系统的长期最优性证明。
